...
- **UX Enhancements (Feb 2026)**: What's New badge on PodcastCards with unplayed episodes, swipe-to-queue/download gestures on episode cards (native only), loading skeletons replacing spinners, buffering indicators on player and mini-player, pre-buffering next queue episode (web only, skipped on native to save memory), auto-retry failed downloads with exponential backoff (10s base, max 2 retries), improved offline indicators (greyed-out play buttons, green available-offline badges, OfflineBanner with "Go to Downloads" link), skip silence setting, press-scale animations on cards, fade tab transitions, springify mini-player animation. All animations optimized for low-end Android devices (slower skeleton pulses, snappier spring configs, reduced network polling to 30s).
