...
const ShiurRow = React.memo(function ShiurRow({ feed, colors }: { feed: Feed; colors: any }) {
  return (
    ...
    >
      {feed.imageUrl ? (
        <Image source={feedImageSource(feed.imageUrl, IMG_CARD)} style={styles.rowImage} contentFit="cover" cachePolicy="memory-disk" recyclingKey={feed.id} transition={180} />
      ) : (
...
export default function AllShiurimScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <Text style={[styles.countText, { color: colors.textSecondary }]}>
        {sortedFeeds.length} {sortedFeeds.length === 1 ? "shiur" : "shiurim"}
      </Text>
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
              {search.trim() ? `No results for "${search}"` : "No shiurim available"}
            </Text>
          </View>
        }
      />
