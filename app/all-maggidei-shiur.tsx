...
import { feedImageSource, IMG_THUMB } from "@/lib/image-resize";
...
const SpeakerCard = React.memo(function SpeakerCard({ speaker, colors }: { speaker: SpeakerGroup; colors: any }) {
  return (
    ...
    >
      {speaker.imageUrl ? (
        <Image source={feedImageSource(speaker.imageUrl, IMG_THUMB)} style={styles.avatar} contentFit="cover" cachePolicy="memory-disk" recyclingKey={speaker.author} transition={180} />
      ) : (
