...
export function SubmitAlumniContactModal({ visible, onClose, onSubmitted, submitterEmail, submitterDisplayName }: Props) {
  ...
  // Doc ID of the existing record (auto-generated by Firestore) — needed
  // so submit() can target the right doc on edit. null = new submission.
  ...
  useEffect(() => {
    if (!visible) return;
    setLoading(true);
    // Pass raw email — fetchMyAlumniContact scans by `email` field
    // (matches website). Earlier this was a doc-ID lookup keyed by
    // lowercased email which missed website-created records.
    fetchMyAlumniContact(submitterEmail)
      .then((existing) => {
        if (existing) {
