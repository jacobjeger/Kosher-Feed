...
const MaggidShiurCard = React.memo(function MaggidShiurCard({ author, feeds, colors }: { author: string; feeds: Feed[]; colors: any }) {
  ...
  return (
    ...
    >
      {imageUrl ? (
        <Image source={feedImageSource(imageUrl, IMG_THUMB)} style={styles.maggidAvatar} contentFit="cover" cachePolicy="memory-disk" recyclingKey={imageUrl} transition={180} />
      ) : (
